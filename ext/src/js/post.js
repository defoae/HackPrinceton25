...
async function getPostIdFromApiByShortcode(sc) {
    const cachedPostId = appCache.postIdInfoCache.get(sc);
    if (cachedPostId) return cachedPostId;
    const apiURL = new URL('/graphql/query/', IG_BASE_URL);
    const fetchOptions = getFetchOptions();
    fetchOptions['method'] = 'POST';
...
async function getPostPhotos(shortcode) {
    ...
    try {
        ...
        if (respone.status === 400) {
            const resolvedId = await getPostIdFromApiByShortcode(shortcode);
            if (!resolvedId) throw new Error('Network bug');
            const apiURL = new URL(`/api/v1/media/${resolvedId}/info/`, IG_BASE_URL);
