...
function resetDownloadState() { return; // UI removed
...
async function saveMedia(media, fileName) { return; // UI removed
    try {
...
async function saveZip() { return; // disabled in Phase 2
...
function setDownloadState(state = 'ready') { return; // UI removed
    const DOWNLOAD_BUTTON = document.querySelector('.download-button');
...
async function handleDownload() { return; // UI removed
    let data = null;
...
function handleLongClick(element, shortClickHandler, longClickHandler, delay = 400) { return; // UI removed
